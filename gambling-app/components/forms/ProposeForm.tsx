...
const ProposeForm = <T extends FieldValues>({ schema, defaultValues, users, bearer }: ProposeFormProps<T>) => {
    ...
    const handleSubmit: SubmitHandler<T> = async () => {
        ...
        fetch(`http://localhost/wp-json/gambling_api/v1/user/weather_suggestion/${parseInt(form.getValues().user_id)}`, requestOptions)
        ...
        <Form {...form}>
            ...
            {cities && (
                <>
                    ...
                    <br />
                    <ol>{cities.map((city, index) => <li key={index}>{index + 1}. {city}</li>)}</ol>
                </>

            )}
            <br />
            {imageIcon && (
                <Image src={imageIcon} width={80} height={80} alt="Weather Icon" className="invert-colors" />
            )}
        </Form>
    );
};
